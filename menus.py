...
class Slider:
    ...
    def __init__(self, x, y, w, label, value, batch, group_bg=None, group_fg=None,
        ...
        self._track_bg   = shapes.Rectangle(x, track_y, w, self.TRACK_H,
                                            color=COLOR_BTN_NORMAL[:3],
                                            batch=batch, group=group_bg)
        self._track_fill = shapes.Rectangle(x, track_y, int(w * self._value),
                                            self.TRACK_H,
                                            color=COLOR_ACCENT[:3],
                                            batch=batch, group=group_fg)
        self._thumb = shapes.Circle(x + int(w * self._value), y + self.THUMB_R,
                                    self.THUMB_R, color=COLOR_ACCENT[:3],
                                    batch=batch, group=group_fg)
        self._label = make_label(label, x - 12, y + self.THUMB_R,
                                 batch, group_fg,
                                 font_name=FONT_UI, font_size=SIZE_SMALL,
                                 color=COLOR_TEXT,
                                 anchor_x="right", anchor_y="center")
        self._val_label = make_label(f"{int(self._value * 100)}%",
    ...
    def _hit(self, mx, my):
        return (abs(mx - self._thumb.x) <= self.THUMB_R + 4 and
...
class OptionsMenu:
    ...
    def __init__(self, width, height, batch, on_back=None):
        self._visible = False
        ...
        self._title   = make_label("OPTIONS", cx, py + self.PANEL_H - 30,
                                   batch, self._grp_fg,
        ...
        self._sliders = []
        configs = [("Musique", 0.70, "music_volume"),
        ...
        for i, (lbl, default, attr) in enumerate(configs):
            ...
            s = Slider(cx + 40, sy, 180, lbl, default,
                       batch, self._grp_mid, self._grp_fg)
            setattr(self, attr, s)
            self._sliders.append(s)

        self._back_btn = Button(cx - 80, py + 20, 160, 44, "← RETOUR",
                                batch, self._grp_mid, self._grp_fg,
                                on_click=on_back)
...
class StatsScreen:
    ...
    def __init__(self, width, height, batch, on_back=None):
        ...
        self._panel   = draw_panel(px, py, self.PANEL_W, self.PANEL_H, batch, self._grp_bg)
        self._borders = draw_border(px, py, self.PANEL_W, self.PANEL_H, 2, batch, self._grp_fg)
        self._title   = make_label("STATISTIQUES", cx, py + self.PANEL_H - 30,
                                   batch, self._grp_fg,
                                   font_name=FONT_TITLE, font_size=SIZE_HEADER,
                                   color=COLOR_ACCENT,
                                   anchor_x="center", anchor_y="center")
        ...
        for i, key in enumerate(stat_keys):
            ...
            kl = make_label(key + "  :", px + 24, row_y,
                            batch, self._grp_fg,
                            font_name=FONT_UI, font_size=SIZE_SMALL,
                            color=COLOR_TEXT_DIM)
            vl = make_label("—", px + self.PANEL_W - 24, row_y,
                            batch, self._grp_fg,
                            font_name=FONT_MONO, font_size=SIZE_SMALL,
                            color=COLOR_TEXT,
        ...
        self._back_btn = Button(cx - 80, py + 16, 160, 44, "← RETOUR",
                                batch, self._grp_mid, self._grp_fg,
                                on_click=on_back)
